...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($seller_products as $seller_product) {
            ...
            $order->code = date('Ymd-His') . random_int(10, 99);
            $order->date = time();
            $order->save();
            ...
            foreach ($seller_product as $cartItem) {
                ...
                if ((int)$product->digital !== 1 && $cartItem['quantity'] > $product_stock->qty) {
   ...
   public function update_tracking_code(Request $request) {
    ...
    public function update_payment_status(Request $request)
    {
        ...
        if ($order->payment_status === 'paid' && (int)$order->commission_calculated === 0) {
            calculateCommissionAffilationClubPoint($order);
        }

        //sends Notifications to user
        NotificationUtility::sendNotification($order, $request->status);
        if ((int)get_setting('google_firebase') === 1 && $order->user->device_token !== null) {
            $request->device_token = $order->user->device_token;
            $request->title = "Order updated !";
