...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        if(!$product->auction_product){
            if(!$product->digital && $request->quantity < $product->min_qty) {
                return array(
                    'status' => 0,
            ...
            if(Cookie::has('referred_product_id') &&
                (int)Cookie::get('referred_product_id') === $product->id) {
            ...
            if($carts && count($carts) > 0){
                ...
                foreach ($carts as $key => $cartItem){
                    ...
                    if((int)$cartItem['product_id'] === (int)$request->id) {
                        $product_stock = $product->stocks->where('variant', $str)->first();
                        $quantity = $product_stock->qty;
    ...
    public function updateQuantity(Request $request)
    {
        ...
        if($cartItem['id'] === $request->id){
            ...
            if(($quantity >= $request->quantity) && $request->quantity >= $product->min_qty) {
                $cartItem['quantity'] = $request->quantity;
            }
